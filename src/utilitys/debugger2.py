...
def noisy_single_bracket(line, text, single_pattern):
    single_bracket = None
    single_bracket_match = re.search(single_pattern, text)
    if single_bracket_match:
        text_start = line.rfind(text)
        single_bracket = text_start + single_bracket_match.start()
    return single_bracket

paths = [str(x) for x in Path("./corpust").glob("**/*.txt")]
        ...
        if not line == "\n" and not line.startswith("{") and not line.endswith("}"):
            ...
            if not noisy_line:
                ...
                for bracket in range(num_of_double_brackets):
                    ...
                    if bracket == 0:
                        # and not bracket == num_of_double_brackets -1:
                        ...
                        if noisy_sentence_active:
                            ...
                            if noisy_sentence.endswith("-"):
                                sentence_in_progress = noisy_sentence.strip("-")
                            else:
                                sentence_in_progress = noisy_sentence + " "
                        ...
                        if not bracket == num_of_double_brackets - 1:
                            ...
                            if single_noisy_match == None:
                                ...
                                if len(before_single_bracket_sentences) >=2:
                                    ...
                                    noisy_text = before_bracket_sentences[0] + " " + line[start_noisy_bracket:end_noisy_bracket] + before_single_bracket + after_single_bracket
